...
function sendMessage() {
  ...
  // Simulate response delay (replace this block with fetch() to Flask)
  const delay = 900 + Math.random() * 600;
  setTimeout(() => {
    removeTypingIndicator();
    const result = getResponse(text);
    // If already escalated, forward the user's message to staff instead of having the bot reply
    if (isEscalated && currentEscalationId) {
      const evt = { id: currentEscalationId, from: 'user', text, time: new Date().toISOString() };
      pushUserMessage(evt);
      // show acknowledgement bubble from system
      appendBotMessage('Your message has been sent to the Guidance Office counselor.', 'neutral');
    } else {
      appendBotMessage(result.response, result.emotion);
      if (result.escalate) {
        // create escalation object and notify staff via localStorage
        ...
        // mark this session as waiting for staff
        ...
        // change placeholder to indicate staff takeover
  ...
  .then(data => {
    removeTypingIndicator();
    appendBotMessage(data.response, data.emotion);
    if (data.escalate) setTimeout(appendEscalationNotice, 400);
  })
  .catch(() => {
    removeTypingIndicator();
